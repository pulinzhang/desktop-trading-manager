...
export function setupAuthHandlers(): void {
  ...
  ipcMain.handle(
    'auth:updateSettings',
    async (_, userId: number, updates: Partial<UserSettings>): Promise<UserSettings> => {
...
export function setupTradeHandlers(): void {
  ...
  ipcMain.handle(
    'db:getTrades',
    async (_, userId: number, sessionId?: number): Promise<Trade[]> => {
      const db = getDatabase()
      if (sessionId) {
        const stmt = db.prepare(
          'SELECT * FROM trades WHERE user_id = ? AND session_id = ? ORDER BY sequence_number ASC'
        )
        return stmt.all(userId, sessionId) as Trade[]
      }
      const stmt = db.prepare(
        'SELECT * FROM trades WHERE user_id = ? ORDER BY sequence_number DESC'
      )
  ...
  ipcMain.handle(
    'db:createTrade',
    async (_, trade: Omit<Trade, 'id' | 'created_at' | 'updated_at'>): Promise<Trade> => {
      const db = getDatabase()
      const stmt = db.prepare(`
  ...
  ipcMain.handle(
    'db:updateTrade',
    async (_, id: number, updates: Partial<Trade>): Promise<Trade | null> => {
      const db = getDatabase()
      ...
      const stmt = db.prepare(
        `UPDATE trades SET ${setClause}, updated_at = CURRENT_TIMESTAMP WHERE id = ?`
      )
      stmt.run(...values)
  ...
  ipcMain.handle(
    'db:clearTrades',
    async (_, userId: number, sessionId?: number): Promise<boolean> => {
      const db = getDatabase()
      if (sessionId) {
...
export function setupSessionHandlers(): void {
  ...
  ipcMain.handle('db:getActiveSession', async (_, userId: number): Promise<Session | null> => {
    const db = getDatabase()
    const stmt = db.prepare(
      'SELECT * FROM sessions WHERE user_id = ? AND is_active = 1 ORDER BY session_number DESC LIMIT 1'
    )
  ...
  ipcMain.handle(
    ...
    async (
      ...
      const lastSession = db
        .prepare('SELECT MAX(session_number) as max_num FROM sessions WHERE user_id = ?')
        .get(userId) as { max_num: number | null } | undefined
  ...
  ipcMain.handle(
    'db:updateSession',
    async (_, sessionId: number, updates: Partial<Session>): Promise<Session | null> => {
      const db = getDatabase()
  ...
  ipcMain.handle('db:resetSessionCounter', async (): Promise<void> => {
...
export function setupExportHandlers(): void {
  ipcMain.handle(
    'export:trades',
    async (_, userId: number, sessionId?: number): Promise<string> => {
      const db = getDatabase()
      let trades: Trade[]
      ...
      if (sessionId) {
        ...
        const stmt = db.prepare(
          'SELECT * FROM trades WHERE user_id = ? ORDER BY sequence_number ASC'
        )
